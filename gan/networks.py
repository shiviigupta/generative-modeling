...
class UpSampleConv2D(torch.jit.ScriptModule):
    ...
    def forward(self, x):
        ...
        ##################################################################
        x_upsampled = x.repeat(1, int(self.upscale_factor**2), 1, 1)
...
class DownSampleConv2D(torch.jit.ScriptModule):
    ...
    def forward(self, x):
        ...
        x_unshuffled = x_unshuffled.view(int(self.downscale_ratio**2), x_unshuffled.shape[0], -1, x_unshuffled.shape[2], x_unshuffled.shape[3])
        x_averaged = x_unshuffled.mean(dim=0)
        return self.conv(x_averaged)
        ##################################################################
        #                          END OF YOUR CODE                      #
        ##################################################################
...
class ResBlockUp(torch.jit.ScriptModule):
    ...
    def __init__(self, input_channels, kernel_size=3, n_filters=128):
        ...
        self.layers = nn.Sequential(
            ...
            nn.ReLU(),
            UpSampleConv2D(n_filters, n_filters=n_filters, kernel_size=3, padding=1)
        )
        self.upsample_residual = UpSampleConv2D(input_channels, n_filters=n_filters, kernel_size=1, padding=0)
        ##################################################################
        #                          END OF YOUR CODE                      #
        ##################################################################
...
class Generator(torch.jit.ScriptModule):
    ...
    def forward_given_samples(self, z):
        ...
        ##################################################################
        x = self.dense(z)
        x = x.view(x.shape[0], 128, self.starting_image_size, self.starting_image_size)
        output = self.layers(x)
        return output
        ##################################################################
        #                          END OF YOUR CODE                      #
    ...
    def forward(self, n_samples: int = 1024):
        ...
        im = self.forward_given_samples(z)
